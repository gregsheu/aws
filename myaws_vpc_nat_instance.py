...
for i in range(0, 3):
    ...
    if i == 0:
        client.create_tags(Resources=[mysubnets[i]], Tags=[{'Key': 'Name', 'Value': myvpc_name + '_public_subnet' + str(i)}])
    else:
        client.create_tags(Resources=[mysubnets[i]], Tags=[{'Key': 'Name', 'Value': myvpc_name + '_private_subnet' + str(i)}])
print(mysubnets)
...
client.associate_route_table(SubnetId=mysubnets[2], RouteTableId=main_route_table_id)
...
resp = client.run_instances(ImageId=nat_image_id, InstanceType='t2.small', KeyName='gregkey', MinCount=1, MaxCount=1, SubnetId=mysubnets[0])
