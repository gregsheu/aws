...
def create_ecs_cluster(cluster_name, cap_arn):
    print('Creating ECS Cluster...')
    ecs_c = boto3.client('ecs')
    resp = ecs_c.create_cluster(clusterName=cluster_name, configuration={'executeCommandConfiguration': {'logging': 'OVERRIDE', 'logConfiguration': {'cloudWatchLogGroupName': 'MyECSCloudWatchLog', 'cloudWatchEncryptionEnabled': False, 's3BucketName': 'ksdevecseast2', 's3EncryptionEnabled': False}}}, capacityProviders=[cap_arn])
...
def create_ecs_service(cluster_name, taskdefinition, target_group_arn, cap_name):
    print('Creating ECS Service...')
    ecs_c = boto3.client('ecs')
    #platformVersion only for Fargate, desiredCount is numbes of task to run
    ecs_c.create_service(cluster=cluster_name, serviceName='MyECSService', taskDefinition=taskdefinition, loadBalancers=[{'targetGroupArn': target_group_arn, 'containerName': 'nginxrtmp', 'containerPort': 80}], desiredCount=8, capacityProviderStrategy=[{'capacityProvider': cap_name, 'weight': 10, 'base': 5}], healthCheckGracePeriodSeconds=10, deploymentController={'type': 'CODE_DEPLOY'}, tags=[{'key': 'Name', 'value': 'MyECSService'}], enableExecuteCommand=True)
...
def main():
    ...
    #Second target group is for blue green deployment in codedeploy
    ...
    #Second listener is for blue green deployment in codedeploy
    ...
    subnet_id = get_pri_subnets(myvpcid)
    create_ecs_auto_scaling_group(subnet_id, launch_template_id, 'MyECSAutoScalingGroup')
    ...
    ecs_name = create_ecs_cluster('MyECSCluster', 'MyECSCapacity')
    ...
    create_ecs_service(ecs_name, taskdefinition, target_group_arn, cap_arn)
